...
{
	Hat_HUD(Hat_PlayerController(GetALocalPlayerController()).MyHUD).OpenHUD(class'Hat_HUDMenu_OnlineChatInputBlock');
}
...
function bool ReceivedNativeInputKey(int ControllerId, name Key, EInputEvent EventType, float AmountDepressed, bool bGamepad)
{
	if (`GameManager.GetCurrentMapFilename() != "titlescreen_final")
	{
		if (Key == 'T')
		{
			...
			{
				Hat_HUD(Hat_PlayerController(GetALocalPlayerController()).MyHUD).OpenHUD(class'Hat_HUDMenu_OnlineChatInput');
			}
